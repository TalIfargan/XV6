...
exit(void)
{
  ...
  curproc->elapsed = ticks - curproc->ctime;
  sched();
  panic("zombie exit");
}
...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      for(p1 = ptable.proc; p1 < &ptable.proc[NPROC]; p1++){
        if(p1->state != RUNNABLE) continue;
        if((p->priority == p1->priority) && (p1->ptime < p->ptime)){
          p = p1;
        }
      }
      if (p->tickstorun == 0){
        p->tickstorun = 1<<(6-p->priority);
      }
...
set_priority(int priority){
  ...
  currproc->ptime = ticks;
  if(priority){
    currproc->tickstorun = 1<<(6-priority);
    }
  else{
    currproc->tickstorun = -1;
    }
